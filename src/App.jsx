...
import { 
  ...
  Zap, Trophy, History as HistoryIcon, ShoppingBag, Coins, Check
} from 'lucide-react';
...
// This reads from Vercel/Netlify Environment Variables
...
// Initialize only if keys exist to prevent build crashes
...
// --- Main App ---
export default function DailyRoutineProFinalXP() {
  ...
  const gamificationStats = useMemo(() => {
    ...
    // 10 XP Coins per hour calculation
  ...
  useEffect(() => {
    ...
    // Listen for User Stats (Wallet, Themes)
  ...
  const handleEndDay = async () => {
    ...
    try {
      ...
      // 2. Award XP Coins (10 per hour)
      const earnedCoins = Math.floor(totalStudyHours * 10);
      if (earnedCoins > 0) {
          const statsRef = doc(db, 'users', user.uid, 'userStats', 'profile');
  ...
  if (!app) return (
    <div className="min-h-screen flex items-center justify-center p-4 text-center flex-col gap-4">
      <h2 className="text-xl font-bold text-red-500">Config Error</h2>
      <p>Please add Firebase keys to Environment Variables.</p>
    </div>
  );
      ...
      <div className="space-y-6 pb-24">
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          ...
          {todayGrades ? (
            ...
            <div className="text-right">
               <div className={`text-2xl font-light text-${themeColor}-600`}>
        ...
        <div className="space-y-4 relative">
          ...
          {schedule.map((item, idx) => {
            const start = getMins(item.time);
            const next = schedule[idx+1] ? getMins(schedule[idx+1].time) : 24*60;
            const isActive = currentMins >= start && currentMins < next;
            const isPast = currentMins >= next;
        ...
        <button 
          ...
          className={`fixed bottom-24 right-4 text-white px-6 py-3 rounded-full shadow-xl flex items-center gap-2 font-bold z-30 hover:scale-105 transition-transform ${todayGrades ? `bg-${themeColor}-600` : 'bg-gray-900'}`}
        >
          {todayGrades ? <RefreshCw size={20} /> : <Award size={20} />} 
          {todayGrades ? "Update Grade" : "Finish Day"}
        </button>
      </div>
    );
  };
  ...
  const renderStudy = () => (
    <div className="space-y-6 pb-24">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-baseline justify-between">
         ...
         <span className={`text-3xl font-extrabold text-${themeColor}-600`}>{getTotalStudy()} <span className="text-sm font-medium text-gray-400">hrs</span></span>
      </div>
        ...
        {SUBJECTS.map(sub => (
          <div key={sub} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className={`text-${themeColor}-600 font-medium text-sm`}>
                {((studyData[sub]?.self || 0) + (studyData[sub]?.own || 0))}h
              </span>
            </div>
            <div className="flex gap-4">
              <div className="flex-1">
                ...
                <input 
                  ...
                  className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-${themeColor}-100`}
                  value={studyData[sub]?.self || ''}
                  onChange={(e) => handleStudyInput(sub, 'self', e.target.value)}
                />
              </div>
              <div className="flex-1">
                ...
                <input 
                  ...
                  onChange={(e) => handleStudyInput(sub, 'own', e.target.value)}
                />
              </div>
            </div>
          </div>
  ...
  const renderDiary = () => (
    <div className="h-[calc(100vh-140px)] flex flex-col pb-24">
      <div className="flex flex-col sm:flex-row justify-between items-end mb-4 px-2 gap-3">
         <div>
            <h2 className="text-2xl font-bold text-gray-800">Daily Diary</h2>
            <p className={`text-${themeColor}-600 font-medium text-sm`}>{currentTime.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric'})}</p>
         </div>
         
         <div className="flex items-center gap-2 bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm self-end sm:self-auto">
            <span className="text-xs font-bold text-gray-400 pl-1 uppercase">Time:</span>
            <input 
              type="time" 
              className={`bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-sm font-medium text-gray-700 outline-none focus:ring-2 focus:ring-${themeColor}-100`}
              value={customDiaryTime}
              onChange={(e) => setCustomDiaryTime(e.target.value)}
            />
            <button 
              onClick={handleInsertTime}
              className={`bg-${themeColor}-100 hover:bg-${themeColor}-200 text-${themeColor}-700 px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1 transition-colors`}
            >
              <Clock size={16} /> Insert
            </button>
         </div>
      </div>
      
      <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col relative group">
        <textarea
          className="flex-1 p-6 w-full resize-none outline-none text-gray-700 leading-relaxed text-lg bg-transparent z-10"
  ...
  const renderHistory = () => (
    <div className="space-y-4 pb-24">
      ...
      {historyLogs.map(log => {
        const totalHrs = Object.values(log.studyData || {}).reduce((a, b) => a + (b.self || 0) + (b.own || 0), 0);
        const overall = log.grades?.overall?.grade || '-';
        return (
          <button 
            key={log.id} 
            onClick={() => setSelectedLog(log)}
            className="w-full bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex justify-between items-center text-left group"
          >
            <div>
              <div className={`font-bold text-gray-800 text-lg group-hover:text-${themeColor}-600 transition-colors`}>
                {new Date(log.date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}
              </div>
              <div className="flex gap-3 text-xs text-gray-500 mt-2 font-medium">
                 <span className="bg-gray-100 px-2 py-1 rounded-md">Routine: {log.grades?.routine?.grade || '-'}</span>
                 <span className="bg-gray-100 px-2 py-1 rounded-md">Study: {totalHrs}h ({log.grades?.study?.grade || '-'})</span>
              </div>
            </div>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white shadow-sm ${getGradeColor(overall)}`}>
               {overall}
            </div>
          </button>
        );
  ...
  const renderAnalytics = () => (
    <div className="space-y-6 pb-24">
       <div className="flex items-center justify-between mb-2 px-2">
         ...
         <div className={`bg-${themeColor}-50 text-${themeColor}-700 px-3 py-1 rounded-full text-xs font-bold border border-${themeColor}-100 flex items-center gap-1`}>
           <Trophy size={14}/> Level {gamificationStats.level}
         </div>
       </div>
       ...
       <div className={`bg-gradient-to-r from-${themeColor}-500 to-${themeColor}-700 rounded-2xl p-6 text-white shadow-lg`}>
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="text-xs font-medium opacity-80 uppercase tracking-wider">Gamer Profile</div>
              <div className="text-2xl font-bold flex items-center gap-2">
                Student Lvl {gamificationStats.level} <Zap className="text-yellow-300 fill-current" size={20}/>
              </div>
            </div>
          </div>
          ...
          <div className="relative pt-1">
            <div className="flex mb-2 items-center justify-between">
              <span className={`text-xs font-semibold inline-block text-${themeColor}-100`}>XP Progress</span>
              <span className={`text-xs font-semibold inline-block text-${themeColor}-100`}>{gamificationStats.xp} / {gamificationStats.nextLevelXp} XP</span>
            </div>
            <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-black/20">
              <div style={{ width: `${gamificationStats.progress}%` }} className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-yellow-400 transition-all duration-1000"></div>
            </div>
          </div>
       </div>
       ...
       <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-700 mb-6 flex items-center gap-2">
            <PieChart size={18} className="text-emerald-500"/> Study Distribution
          </h3>
       ...
       <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
            <Calendar size={18} className="text-blue-500"/> Consistency Map (30 Days)
          </h3>
      ...
      <header className="bg-white border-b border-gray-100 px-4 py-3 sticky top-0 z-20">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-lg border border-gray-200">
